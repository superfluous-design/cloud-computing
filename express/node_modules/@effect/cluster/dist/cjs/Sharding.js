...
const make = /*#__PURE__*/Effect.gen(function* () {
  ...
  if (Option.isSome(config.runnerAddress)) {
    ...
    }), Effect.catchAllCause(cause => Effect.logError("Could not refresh shard locks", cause).pipe(Effect.andThen(clearSelfShards))), Effect.delay("10 seconds"), Effect.forever, Effect.interruptible, Effect.forkIn(shardingScope));
          ...
          if (!state) {
            if (message._tag === "IncomingRequest") {
              return Effect.orDie(message.respond(Reply.ReplyWithContext.fromDefect({
                ...
                defect: new _ClusterError.EntityNotManagedByRunner({
                  address
                })
              })));
            }
  ...
  yield* Effect.gen(function* () {
    ...
    }).pipe(Effect.intoDeferred(startedLatch), Effect.forkScoped);
