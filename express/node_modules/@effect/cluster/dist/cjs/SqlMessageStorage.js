...
`.pipe(Effect.tapDefect(error => Effect.annotateLogs(Effect.logDebug("Failed to create indexes", error), {
  ...
  return yield* MessageStorage.makeEncoded({
    ...
    sql`
        ...
        ORDER BY rowid ASC
      `.unprepared.pipe(Effect.provideService(SqlClient.SafeIntegers, true), Effect.map(Arr.map(row => Number(row.kind) === replyKind.WithExit ? {
      _tag: "WithExit",
      id: String(row.id),
      requestId: String(row.request_id),
    ...
    })), _ClusterError.PersistenceError.refail, withTracerDisabled),
