...
export const rollback = ({ dbState, dbEventlog, eventNumsToRollback, }) => Effect.gen(function* () {
    const rollbackEvents = dbState
        .select(sql `SELECT * FROM ${SystemTables.SESSION_CHANGESET_META_TABLE} WHERE (seqNumGlobal, seqNumClient) IN (${eventNumsToRollback.map((id) => `(${id.global}, ${id.client})`).join(', ')})`)
        .map((_) => ({
    ...
    const eventNumPairChunks = ReadonlyArray.chunksOf(100)(eventNumsToRollback.map((seqNum) => `(${seqNum.global}, ${seqNum.client})`));
    // Delete the changeset rows
    for (const eventNumPairChunk of eventNumPairChunks) {
        dbState.execute(sql `DELETE FROM ${SystemTables.SESSION_CHANGESET_META_TABLE} WHERE (seqNumGlobal, seqNumClient) IN (${eventNumPairChunk.join(', ')})`);
    }
    // Delete the eventlog rows
    for (const eventNumPairChunk of eventNumPairChunks) {
        dbEventlog.execute(sql `DELETE FROM ${SystemTables.EVENTLOG_META_TABLE} WHERE (seqNumGlobal, seqNumClient) IN (${eventNumPairChunk.join(', ')})`);
    }
